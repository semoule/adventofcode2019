...
#dataset = [3, 0, 4, 0, 99]
#dataset = [ 1002, 4, 3, 4, 33 ]
dataset = [3, 21, 1008, 21, 8, 20, 1005, 20, 22, 107, 8, 21, 20, 1006, 20, 31, 1106, 0, 36, 98, 0, 0, 1002, 21, 125, 20, 4, 20, 1105, 1, 46, 104, 999, 1105, 1, 46, 1101, 1000, 1, 20, 4, 20, 1105, 1, 46, 98, 99]
...
#    Opcode 8 is equals: if the first parameter is equal to the second parameter, it stores 1 in the position given by the third parameter. Otherwise, it stores 0.
    ...
    if int(opmode[2]) < int(opmode[1]):
      dataset[int(opmode[0])] = 1
    else:
      dataset[int(opmode[0])] = 0
    pointer = pointer + 1
