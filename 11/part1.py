...
def intcode(dataset, pointer, inputval):
  relativebase = 0
  outindex = 0
  lastmove = False
  outval = [0, 0]
...
#  print()
  ...
  return outval[0], outval[1], dataset, pointer, lastmove

def move_robot(dataset, pointer, coordinate, direction):
  #inputval is color
  inputval = coordinate[2]

  # robot brain
  result = intcode(dataset, pointer, inputval)
  color = result[0]
  ...
  return dataset, pointer, newcoordinate, color, newdirection, lastmove
...
while not lastmove:
  origin = coordinate
  result = move_robot(dataset, pointer, coordinate, direction)
  dataset = result[0]
  pointer = result[1]
  coord = result[2]
  color = result[3]
